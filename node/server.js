...
ChatServer.prototype.sendMessage = function (socket, data) {
    ...
    if (!user || !user.hasOwnProperty('userId') || !user.hasOwnProperty('userName')) {
...
ChatServer.prototype.setAvailability = function (socket, data) {
    let user = socket.user;
    if (!user || !data || !data.hasOwnProperty('isAvailable')) {
        return;
    }
